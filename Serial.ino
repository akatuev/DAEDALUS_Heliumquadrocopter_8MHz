...
static void inline SerialOpen(uint8_t port, uint32_t baud) {  // baud ist hier 115200

// Wir wollen Baudrate 9600 zum Test: UBRR = 103
/*  uint8_t h = ((F_CPU  / 4 / baud -1) / 2) >> 8;
  uint8_t l = ((F_CPU  / 4 / baud -1) / 2);
  uint8_t h = 103 >> 8; // 9600 funzt
  uint8_t l = 103;      // 9600 funzt
  */
  // UBRR = {f_osc/(8*BAUD)} -1
  uint8_t h = ((F_CPU/(8*baud))-1) >> 8;
  uint8_t l = ((F_CPU/(8*baud))-1);
  
  
  switch (port) {
    #if defined(PROMINI)
      case 0:
              UCSR0A  = (1<<U2X0); // U2X0 Bit auf 1 bedeutet doppelte 
              UBRR0H = h; 
              UBRR0L = l; 
